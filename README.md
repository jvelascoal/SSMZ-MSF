...
```bash
python ssmz_msf_full_v3_3.py grid|graph <data_file> <alpha> <mode> [h1|h2] [time] [draw] [output] [plot]
```
...
python ssmz_msf_full_v3_3.py grid examples/field_6x7.txt 0.5 h2 draw output

# Grid, lazy exact model, warm-started with H2, 300 s budget, full output
python ssmz_msf_full_v3_3.py grid examples/field_6x7.txt 0.5 lazy h2 300 draw output plot

# Grid, flow exact model without warm start, 600 s budget
python ssmz_msf_full_v3_3.py grid examples/field_6x7.txt 0.7 flow 600

# Graph mode on a general planar instance, heuristic H2
python ssmz_msf_full_v3_3.py graph examples/graph_irregular.txt 0.7 h2 draw output
```

Running `python ssmz_msf_full_v3_3.py` without arguments prints the full usage help.
